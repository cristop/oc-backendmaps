<?php namespace Cristo\BackendMaps\FormWidgets;
...
class BackendMaps extends FormWidgetBase
{
    ...
    private function _setFromSettings()
    {
        ...
        if(isset($settingsInstance->attributes['default_location'])){
          $latLong = explode(',', $settingsInstance->attributes['default_location']);
        }

        if(isset($settingsInstance->attributes['address_map_key'])){
          $this->apiKey = $settingsInstance->attributes['address_map_key'] ?? '';
          $this->latitude = $latLong[0] ?? '39.02610162777133';
          $this->longitude = $latLong[1] ?? '1.4830825124974565';
        }
    }
}
